...
Logger::Logger(std::ostream &stream, std::wostream &wstream)
    : _stream(&stream)
    , _wstream(&wstream) {
#if defined(WIN32)
...
static bool shouldPrintFunctionName(Level level) {
    return level == Level::critical || level == Level::error || level == Level::debug ||
           level == Level::trace;
}

void Logger::log_Unsafe(Level level, const std::string &message,
                        const std::source_location &sourceLocation) {
    const std::string functionLine =
        shouldPrintFunctionName(level)
...
void Logger::log_Unsafe(Level level, const std::wstring &message,
    ...
    const std::wstring functionLine = [&]() -> std::wstring {
        if (shouldPrintFunctionName(level)) {
            ...
            return std::format(L"[{}:{}] ", std::wstring(funcName, funcName + strlen(funcName)),
                               sourceLocation.line());
        }
        return std::wstring();
    }();

    const std::wstring formatedMessage = std::format(
        L"[{:%Y-%m-%d %H:%M:%S}] [{}] {} {}", std::chrono::system_clock::now(),
        std::bit_cast<const wchar_t *>(getLevelName_Unsafe(level)), message, functionLine);
...
const char *Logger::getLevelName_Unsafe(Level level) const {
    static const std::unordered_map<Level, const char *> levelNames{
        {Level::critical, "CRITICAL"}, {Level::error, "ERROR"}, {Level::warning, "WARNING"},
        {Level::info, "INFO"},         {Level::debug, "DEBUG"}, {Level::trace, "TRACE"},
    };
